  ...
  .then(async ipfs => {
    const run = async () => {
      try {
        ...
        console.log(
          `Creating ${numberOfEntries} documents for updating. Stand by! `
        );
        ...
        setInterval(() => {
          ...
          console.log(
            `${queriesPerSecond} queries per second, ${totalQueries} queries in ${seconds} seconds (Oplog: ${db._oplog.length})`
          );
          queriesPerSecond = 0;
        }, 1000);
        // Start the main loop
